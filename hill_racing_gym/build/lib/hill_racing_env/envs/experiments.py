...
def eval_model_single(model_path, monitor_name, action_space, reward_type, reward_function, episodes):
    env = gym.make(env_id, action_space=action_space, reward_type=reward_type,
                   reward_function=reward_function)
    env = Monitor(env, monitor_name, info_keywords=("score", ))
...
if __name__ == "__main__":
    ...
    # # RERUN using single: Evaluation of discrete action space with action based rewards 1000 soft -150 difficulty
    eval_model_single(model_path="baseline_models/ppo_base_action_soft_1000_3.zip",
                      monitor_name="eval_ppo_discrete_action_soft_1000", action_space="discrete_3",
                      reward_type="soft", reward_function="action", episodes=1000)
